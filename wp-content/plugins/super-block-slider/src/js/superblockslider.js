...
(function () {
  ...
  var superblockslider = function (superblocksliderSlides) {
    var superblocksliderSlider = document.querySelectorAll(
      superblocksliderSlides
    );
    superblocksliderSlider.forEach(function (slider, index) {
      var initialActiveSlideAttr = slider.getAttribute(
      ...
      var transitionDurationAttr = slider.getAttribute(
      ...
      var animating = false;
      var el_superblockslider__track = slider.querySelector(
        ".superblockslider__track"
      );
      var el_superblockslider__slides = slider.querySelectorAll(
        ".superblockslider__slide"
      );
      var el_superblockslider__button__previous = slider.querySelector(
        ".superblockslider__button__previous"
      );
      var el_superblockslider__button__next = slider.querySelector(
        ".superblockslider__button__next"
      );
      el_superblockslider__track.addEventListener(
        "transitionstart",
        transitionStart
      );
      el_superblockslider__track.addEventListener(
      ...
      var parallaxSlides = slider.querySelectorAll(
        '.superblockslider__slide[data-parallax="true"]'
      );
      if (parallaxSlides) {
        parallaxInit();
        window.addEventListener("scroll", function (event) {
          parallaxSlides.forEach(function (parallaxSlide, index) {
            var parallaxAttribute = parallaxSlide.getAttribute(
              "data-parallax-speed"
            );
            var el_slide_bg = parallaxSlide.querySelectorAll(
            ...
            if (parallaxAttribute) {
              ...
              if (
                ...
                var parallaxSpeed = parallaxAttribute
      ...
      function parallaxInit() {
        parallaxSlides.forEach(function (slide, index) {
          ...
          var el_slide_bg = slide.querySelectorAll(
            ".superblockslider__slide__bg"
          )[0];
      ...
      var el_superblockslider__buttons = slider.querySelectorAll(
      ...
      function animateTrackToSlideId(slideId, toggleAutoplay) {
        ...
        if (!animating) {
          ...
          if (currentSlideId != slideId) {
            el_superblockslider__slides = slider.querySelectorAll(
              ".superblockslider__slide"
            );
            ...
            if (settings.loopSlide == false) {
              ...
              if (
                currentSlideIndex === 0 &&
                el_superblockslider__slides.length > 2
              ) {
                el_superblockslider__track.style.transition = "none";
                var lastSide =
                  el_superblockslider__slides[
                    el_superblockslider__slides.length - 1
                  ];
              ...
              var slideMatch = slider.querySelectorAll(
                '[data-slide-index="' + slideId + '"]'
              );
              if (slideMatch[0] && slideMatch[0].parentNode) {
                var slideMatch_parent_children =
                  slideMatch[0].parentNode.children;
                var closeSlide = Array.from(slideMatch_parent_children).indexOf(
                  slideMatch[0]
                );
      ...
      function animate(slideId, slideIndex) {
        if (settings.transitionEffect == "slide") {
          el_superblockslider__track.style.transition =
      ...
      function transitionStart() {
        ...
        slider
          ...
          .classList.add("superblockslider__slide--animating-in");
        slider
          .querySelector('[data-slide-index="' + previousSlideId + '"]')
          .classList.add("superblockslider__slide--animating-out");
      }
      function transitionEnd() {
        slider
          .querySelector(".superblockslider__slide--active")
          .classList.remove("superblockslider__slide--active");
        slider
          .querySelector('[data-slide-index="' + currentSlideId + '"]')
          .classList.add("superblockslider__slide--active");
        if (settings.slideNavigation != "none") {
          slider
            .querySelector(".superblockslider__button--active")
            .classList.remove("superblockslider__button--active");
          el_superblockslider__buttons[currentSlideId].classList.add(
      ...
      if (
        el_superblockslider__button__previous &&
        el_superblockslider__button__next
      ) {
        el_superblockslider__button__previous.addEventListener(
          "click",
          function () {
            prevSlide(null, true);
          }
        );
        el_superblockslider__button__next.addEventListener(
          "click",
          function () {
            nextSlide(null, true);
          }
      ...
      function removeAnimatingClasses() {
        slider
          .querySelector('[data-slide-index="' + currentSlideId + '"]')
          .classList.remove("superblockslider__slide--animating-in");
        slider
          .querySelector('[data-slide-index="' + previousSlideId + '"]')
          .classList.remove("superblockslider__slide--animating-out");
      }
      if (settings.variableHeight) {
      ...
      function updateSliderHeight() {
        ...
        var currentImage = slider.querySelector(
          '[data-slide-index="' +
            currentSlideId +
            '"] img.visible--' +
            currentSceenSize
        ...
        if (currentImage) {
          ...
          var imageVariableHeight = calculateVariableHeight(
            imageOriginalWidth,
            sliderWidth,
            imageOriginalHeight
          );
