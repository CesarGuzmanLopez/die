 ...
 */
function ctemawp_breadcrumbs_sources( $options ) {
...
add_filter( 'ctemawp_breadcrumbs_source_list', 'ctemawp_breadcrumbs_sources' );
 ...
 * @return void
 */
function ctemawp_breadcrumb_trail( $args = array() ) {
	...
	$breadcrumb_source = get_theme_mod( 'ctema_breadcrumbs_source' );
	...
	if ( function_exists( 'yoast_breadcrumb' ) && true === $wpseo_data['breadcrumbs-enable'] && $breadcrumb_source && 'yoast-seo' === $breadcrumb_source ) {
		...
		if ( $breadcrumbs_position = get_theme_mod( 'ctema_breadcrumbs_position' ) ) {
	...
	$schema = get_theme_mod( 'ctema_breadcrumb_schema', true );

	$defaults = array(
		'schema' => $schema,
	);

	$args       = apply_filters( 'ctemawp_breadcrumb_trail_args', $defaults );
	$breadcrumb = apply_filters( 'ctemawp_breadcrumb_trail_object', null, $args );

	if ( ! is_object( $breadcrumb ) ) {
		$breadcrumb = new CtemaWP_Breadcrumb_Trail( $args );
	}
...
function owp_woo_breadcrumbs( $args ) {
	...
	$separator = apply_filters( 'ctemawp_breadcrumb_separator', get_theme_mod( 'ctema_breadcrumb_separator', '>' ) );
 ...
 * @access public
 */
class CtemaWP_Breadcrumb_Trail {
	...
	/**
	 * Sets up the breadcrumb trail properties.  Calls the `CtemaWP_Breadcrumb_Trail::add_items()` method
	 ...
	 *     @type array     $labels         Text labels. @see CtemaWP_Breadcrumb_Trail::set_labels()
	 *     @type array     $post_taxonomy  Taxonomies to use for post types. @see CtemaWP_Breadcrumb_Trail::set_post_taxonomy()
	...
	public function __construct( $args = array() ) {
		...
		$this->args = apply_filters( 'ctemawp_breadcrumb_trail_args', wp_parse_args( $args, $defaults ) );
	...
	public function get_trail() {
		...
		if ( 0 < $item_count ) {
			...
			// Postion class.
			if ( '' !== get_theme_mod( 'ctema_breadcrumbs_position' ) ) {
				$p_class = ' position-' . get_theme_mod( 'ctema_breadcrumbs_position' );
			}
	...
	protected function set_labels() {
		...
		$defaults = array(
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'ctemawp' ),
			'home'                => get_theme_mod( 'ctema_breadcrumb_translation_home', esc_html__( 'Home', 'ctemawp' ) ),
			'error_404'           => get_theme_mod( 'ctema_breadcrumb_translation_error', esc_html__( '404 Not Found', 'ctemawp' ) ),
			'archives'            => esc_html__( 'Archives', 'ctemawp' ),
			// Translators: %s is the search query. The HTML entities are opening and closing curly quotes.
			'search'              => get_theme_mod( 'ctema_breadcrumb_translation_search', esc_html__( 'Search results for', 'ctemawp' ) ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s', 'ctemawp' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s', 'ctemawp' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s', 'ctemawp' ),
	...
	protected function set_post_taxonomy() {
		...
		$this->post_taxonomy = apply_filters( 'ctemawp_breadcrumb_trail_post_taxonomy', wp_parse_args( $this->args['post_taxonomy'], $defaults ) );
	}
	...
	protected function add_items() {
		...
		$this->items = array_unique( apply_filters( 'ctemawp_breadcrumb_trail_items', $this->items, $this->args ) );
	}
	...
	protected function add_network_home_link() {
		...
		// Home item
		$setting = get_theme_mod( 'ctema_breadcrumb_home_item', 'icon' );
		...
		if ( is_customize_preview() || 'icon' == $setting ) {
			$icon = ctemawp_icon( 'home', false );
		}
		if ( is_customize_preview() && 'text' == $setting ) {
			$icon = ctemawp_icon( 'home', false, 'icon-home has-text' );
		}
	...
	protected function add_singular_items() {
		...
		// Post type taxonomy
		$posts_tax     = get_theme_mod( 'ctema_breadcrumb_posts_taxonomy', 'category' );
		$products_tax  = get_theme_mod( 'ctema_breadcrumb_products_taxonomy', 'shop' );
		$portfolio_tax = get_theme_mod( 'ctema_breadcrumb_portfolio_taxonomy', 'ctema_portfolio_category' );
		...
		// If the post type is 'ctema_portfolio'.
		else if ( class_exists( 'Ctema_Portfolio' )
			&& 'ctema_portfolio' === $post->post_type
			&& 'none' != $portfolio_tax ) {
		...
		if ( true == get_theme_mod( 'ctema_breadcrumb_show_title', true )
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'ctemawp' ) ) );
		}
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'ctemawp' ) ) );
		}
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] ) {
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'ctemawp' ) ) );
		}
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'ctemawp' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'ctemawp' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'ctemawp' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'ctemawp' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'ctemawp' ) ) );
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'ctemawp' ) ) ) );
				}
				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'ctemawp' ) ) ) );
				}
				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag ) {
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'ctemawp' ) ) ) );
				}
