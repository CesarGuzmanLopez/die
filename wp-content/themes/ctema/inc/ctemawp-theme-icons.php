...
if ( ! function_exists( 'ctemawp_theme_icons' ) ) {
	 ...
	 * @since 2.0
	 */
	function ctemawp_theme_icons() {

		$ctemawp_theme_icons = array(
			'home'                   => array(
				'sili' => 'icon-home',
				'fai'  => 'fas fa-home',
				'svg'  => 'homepage',
		...
		return apply_filters( 'ctemawp_theme_icons', $ctemawp_theme_icons );
	}
}
 ...
 * @since 2.0
 */
if ( ! function_exists( 'ctemawp_theme_icon_class' ) ) {

	function ctemawp_theme_icon_class() {

		$i_class = get_theme_mod( 'ctema_theme_default_icons', 'sili' );
		...
		if ( 'sili' === $i_class ) {
			...
			return __( 'Please select an icon type.', 'ctemawp' );
		}

		return apply_filters( 'ctemawp_theme_icon_class', $ti_class );
	}
}
 ...
 * @since 2.0
 */
if ( ! function_exists( 'ctemawp_theme_blog_meta_separator' ) ) {

	function ctemawp_theme_blog_meta_separator() {

		$meta_style = get_theme_mod( 'ctema_blog_meta_separator', 'default' );

		$meta_style = $meta_style ? $meta_style : 'default';

		return apply_filters( 'ctemawp_theme_blog_meta_separator', $meta_style );
	}
}
...
if ( ! function_exists( 'ctemawp_theme_single_post_separator' ) ) {
	...
	function ctemawp_theme_single_post_separator() {
		...
		return apply_filters( 'ctemawp_theme_single_post_separator', $sep_style );
	}
}

function get_ctema_icon( $args = array() ) {
 ...
 * Display the icons
 */
if ( ! function_exists( 'ctemawp_print_icon' ) ) {

	function ctemawp_print_icon( $args = array(), $echo = true ) {
		if ( empty( $args ) ) {
			return __( 'Please define default parameters in the form of an array.', 'ctemawp' );
		}

		$icon      = '';
		$icon_type = get_theme_mod( 'ctema_theme_default_icons', 'sili' );

		if ( isset( $icon_type ) && ! empty( $icon_type ) ) {

			if ( 'sili' === $icon_type || 'fai' === $icon_type ) {
				$icon = get_ctema_icon( $args );
			} elseif ( 'svg' === $icon_type ) {
				$icon = ctema_svg_print_icon( $args, false );
			} else {
				return __( 'Please define an icon type.', 'ctemawp' );
			}
		}

		$icon = apply_filters( "ctemawp_print_icon_{$icon}", $icon );
 ...
 *
 * @return string CtemaWP Icon.
 */
if ( ! function_exists( 'ctemawp_icon' ) ) {

	function ctemawp_icon( $icon, $echo = true, $class = '', $title = '', $desc = '', $aria_hidden = true, $fallback = false ) {

		$icon = ctemawp_print_icon(
			array(
				'icon'        => $icon,
				'class'       => $class,
				'title'       => $title,
		...
		$icon = apply_filters( "ctemawp_icon_{$icon}", $icon );
