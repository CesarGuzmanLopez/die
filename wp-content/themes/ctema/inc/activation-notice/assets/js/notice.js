...
jQuery(document).ready(function ($) {
	...
	var owp_notification = {
		...
		init: function () {
			...
			if ($('.owp-ctema-extra-plugin-inner').length) {
				this.render_actions();
				$(document).on('click', '.owp-ctema-extra-plugin-inner .notice-dismiss', this.dismiss_notice);
				$(document).on('click', '.owp-ctema-extra-plugin-inner .notice-actions button', this.main_action);
			}
		...
		render_actions: function () {
			$.ajax({
				...
				url: ajaxurl,
				data: {
					action: 'ctemawp_check_notice_actions'
				},
		...
		dismiss_notice: function (evt) {
			...
			$(evt.target).closest('.notice-ctema-extra-plugin').remove();

			$.ajax(ajaxurl, {
				type: 'POST',
				data: {
					action: 'ctemawp_dismissed_notice',
				},
			})
		...
		main_action: function (evt) {
			...
			$.ajax({
				...
				url: ajaxurl,
				data: {
					action: 'ctemawp_notice_button_click'
				},
				success: function (response) {
					if (response.success) {
