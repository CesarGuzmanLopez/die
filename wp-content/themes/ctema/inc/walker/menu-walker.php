...
if ( ! class_exists( 'CtemaWP_Custom_Nav_Walker' ) ) {
	class CtemaWP_Custom_Nav_Walker extends Walker_Nav_Menu {
		/**
		...
		public function start_lvl( &$output, $depth = 0, $args = null ) {
			...
			if ( $depth === 0 && $this->megamenu != '' && 'full_screen' != ctemawp_header_style() && 'vertical' != ctemawp_header_style() ) {
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			if ( $depth === 0 && $this->has_children && 'vertical' != ctemawp_header_style() ) {
			...
			if ( $item->category_post != '' && $item->object == 'category' && 'vertical' != ctemawp_header_style() ) {
			...
			if ( ( $item->template || $item->mega_template ) && $this->megamenu != '' && 'vertical' != ctemawp_header_style() ) {
				ob_start();
					include CTEMAWP_INC_DIR . 'walker/template.php';
					$template_content = ob_get_contents();
				ob_end_clean();
				$item_output .= $template_content;
			}

			if ( $item->megamenu_widgetarea && $this->megamenu != '' && 'vertical' != ctemawp_header_style() ) {
				ob_start();
			...
			if ( $depth === 0 && $item->category_post != ''
				...
				if ( $item->category_post != '' && $item->object == 'category' ) {
					...
					foreach ( $sub_categories as $category ) {
						...
						// Title
						$output .= '<h3 class="mega-cat-title">' . esc_html__( 'Latest in', 'ctemawp' ) . ' ' . get_cat_name( $cat_id ) . '</h3>';
						...
						while ( $cat_query->have_posts() ) {
							...
							if ( has_post_thumbnail() ) {
								...
								// Image args.
								$img_args = array(
									'alt' => esc_attr( ctemawp_theme_strings( 'owp-string-read-more-article', false ) ) . ' ' . get_the_title(),
								);
								...
								$output .= '<h3 class="mega-post-title"><a href="' . get_permalink() . '">' . get_the_title() . '</a></h3><div class="mega-post-date">' . ctemawp_icon( 'date', false ) . ' ' . get_the_date() . '</div>';
		...
		public function display_element( $element, &$children_elements, $max_depth, $depth, $args, &$output ) {
			...
			if ( ! empty( $children_elements[ $element->$id_field ] ) && ( $depth == 0 )
				...
				if ( true == get_theme_mod( 'ctema_menu_arrow_down', true ) ) {
					( 'full_screen' == $header_style ) ? $element->title .= '' . $full_screen_dropdown : $element->title .= ctemawp_icon( 'angle_down', false, 'nav-arrow' );
				}
			}
			...
			if ( ! empty( $children_elements[ $element->$id_field ] ) && ( $depth > 0 ) ) {
				$element->classes[] = 'dropdown';
				if ( true == get_theme_mod( 'ctema_menu_arrow_side', true ) ) {
					if ( is_rtl() ) {
						( 'full_screen' == $header_style ) ? $element->title .= $full_screen_dropdown : $element->title .= ctemawp_icon( 'angle_left', false, 'nav-arrow' );
					} else {
						( 'full_screen' == $header_style ) ? $element->title .= $full_screen_dropdown : $element->title .= ctemawp_icon( 'angle_right', false, 'nav-arrow' );
					}
				}
			}
