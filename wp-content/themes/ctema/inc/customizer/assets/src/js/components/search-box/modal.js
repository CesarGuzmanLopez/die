...
const SearchBoxModal = ( { ...props } ) => {
    ...
    const switchThemeMode = () => {
        ...
        ctemaCustomizerSearchOptions.darkMode = ctemaCustomizerSearchOptions.darkMode === '1'
        || ctemaCustomizerSearchOptions.darkMode === 'true'
        || ctemaCustomizerSearchOptions.darkMode === true ? false : true;
        jQuery( '.modal-dialog.ctema-customize-search-modal' ).toggleClass( 'light-theme' );

        wp.ajax.post( 'ctema_update_search_box_light_mode', {
            darkMode: ctemaCustomizerSearchOptions.darkMode
        } );
    }
    ...
    const ClickHandler = ( sectionName, ElementID ) => {
        ...
        section.expand();
        setsearchResults( '' );
        jQuery( '.ctema-customize-search-modal' ).parent().fadeOut();
        jQuery( '.modal-backdrop.show' ).removeClass( 'show' );
        props.onHide();

        setTimeout( function() {
            jQuery( '.ctema-customize-search-modal .modal-header button.close' ).trigger( 'click' ).trigger( 'mouseup' )
            if ( jQuery( '#customize-control-' + ElementID ).length ) {
                ...
                jQuery( '#customize-control-' + ElementID ).addClass( 'ctema-control-focused' );
            }
        }, 1500 )

        setTimeout( () => {
            jQuery( '.ctema-control-focused' ).removeClass( 'ctema-control-focused' );
        }, 8000 );
    }
    ...
    return <>
        <Modal
            ...
            onHide = { props.onHide }
            dialogClassName = {
                ctemaCustomizerSearchOptions.darkMode === '1'
                || ctemaCustomizerSearchOptions.darkMode === 'true'
                || ctemaCustomizerSearchOptions.darkMode === true ? "ctema-customize-search-modal" : "ctema-customize-search-modal light-theme" }
            aria-labelledby = "contained-modal-title-vcenter"
            centered>
            <Modal.Header closeButton>
                <Form.Group className="full-width" controlId="ctema-wp-customize-search-input">
