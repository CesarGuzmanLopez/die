...
// Start and run class.
if ( ! class_exists( 'CtemaWP_Gutenberg_Editor' ) ) {

	/**
	 * CtemaWP Gutenberg Class
	 */
	class CtemaWP_Gutenberg_Editor {
		...
		public function elements() {
			...
			return apply_filters(
				'ctema_gutenberg_typography_tags',
				array(
					'body'            => array(
		...
		public function loop( $return = 'css' ) {
			...
			if ( ! empty( $elements ) ) {
				...
				if ( 'css' === $return && ! empty( $css ) ) {
					$css = '/* CtemaWP Gutenberg Style */' . $css;
					return $css;
				}
		...
		public function gutenberg_editor_style( $output ) {
			...
			if ( ! is_null( get_current_screen() ) && get_current_screen()->is_block_editor() ) {
				...
				$heading_tag = get_theme_mod( 'ctema_single_post_heading_tag', 'h2' );
				...
				$background_layout           = get_theme_mod( 'ctema_main_layout_style', 'wide' );
				$background_color            = get_theme_mod( 'ctema_background_color', '#ffffff' );
				$boxed_separate_bg_color     = get_theme_mod( 'ctema_boxed_inner_bg', '#ffffff' );
				...
				if ( ! empty( $gutenberg_css ) ) {
					echo '<style id="ctema-gutenberg-style"> ' . esc_attr( $gutenberg_css ) . '</style>';
				}
			}
...
return new CtemaWP_Gutenberg_Editor();
