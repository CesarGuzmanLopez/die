...
if ( ! class_exists( 'CtemaWP_AMP' ) ) {
	...
	class CtemaWP_AMP {
		...
		public function __construct() {
			...
			add_action( 'wp', array( $this, 'ctemawp_amp_config' ) );
		 ...
		 * @return void
		 */
		public function ctemawp_amp_config() {

			if ( ! CTEMAWP_Theme_Class::ctemawp_is_amp() ) {
				return;
			}

			add_filter( 'ctemawp_schema_location_html', array( $this, 'ctemawp_body_id' ) );
			add_filter( 'ctemawp_menu_toggle_data_attrs', array( $this, 'ctemawp_menu_taggle_attrs' ) );
			add_action( 'wp_footer', array( $this, 'render_amp_states' ) );
			add_filter( 'ctemawp_attrs_mobile_dropdown', array( $this, 'ctemawp_dropdown_menu_wrapper' ) );
			add_filter( 'ctemawp_attrs_mobile_fullscreen', array( $this, 'ctemawp_fullscreen_menu_wrapper' ) );
			add_filter( 'ctemawp_attrs_mobile_fullscreen_close', array( $this, 'ctemawp_fullscreen_menu_close' ) );
			add_filter( 'walker_nav_menu_start_el', array( $this, 'mobile_submenu_icon' ), 20, 4 );
			add_filter( 'ctemawp_toggle_submenu_icon', array( $this, 'toggle_submenu_icon' ), 20, 2 );
			add_filter( 'owp_attr_submenu-toggle', array( $this, 'menu_toggle_button' ), 20, 3 );
			add_filter( 'ctemawp_attrs_search_bar', array( $this, 'ctemawp_menu_search_bar' ) );
			add_filter( 'ctemawp_attrs_overlay_search_bar', array( $this, 'ctemawp_menu_overlay_search_bar' ) );
			add_filter( 'ctemawp_attrs_nav_search_bar', array( $this, 'ctemawp_nav_item_search_bar' ) );
			add_filter( 'ctemawp_attrs_main_nav', array( $this, 'ctemawp_attrs_nav_wrapper' ) );
			add_filter( 'ctemawp_attrs_vertical_header_style', array( $this, 'ctemawp_attrs_vertical_header_toggle' ) );
			add_filter( 'ctema_head_css', array( $this, 'ctemawp_amp_css' ) );

			// Woocommerce AMP.
			if ( CTEMAWP_WOOCOMMERCE_ACTIVE ) {

				add_filter( 'ctema_menu_cart_style', array( $this, 'ctemawp_amp_mini_cart_style' ) );
		 ...
		 * @return String
		 */
		public function ctemawp_body_id( $schema ) {
			return $schema . 'id="ctemawp-body"';
		}
		 ...
		 */
		public function ctemawp_amp_css( $output ) {
			...
			$amp_menu_breakpoint        = get_theme_mod( 'ctema_mobile_menu_breakpoints', '959' );
			$amp_menu_custom_breakpoint = get_theme_mod( 'ctema_mobile_menu_custom_breakpoint' );
			$amp_vheader_style_width    = get_theme_mod( 'ctema_vertical_header_width', '300' );
			...
			.fullscreen-mobile .ctemawp-mobile-menu-icon a.mobile-menu.opened .ctemawp-text {
				display: inherit;
			}';
		 ...
		 * @return string
		 */
		public function ctemawp_menu_taggle_attrs( $input ) {
			$input .= ' on="tap:AMP.setState( { owpAmpMenuExpanded: ! owpAmpMenuExpanded } ),ctemawp-body.toggleClass(class=mobile-dropdown)" ';
		 ...
		 * @return string
		 */
		public function ctemawp_dropdown_menu_wrapper( $input ) {
		 ...
		 * @return string
		 */
		public function ctemawp_fullscreen_menu_wrapper( $input ) {
			$input .= ' [class]="\'clr\' + ( owpAmpMenuExpanded ? \' opened\' : \'\' )" ';
			$input .= ' aria-expanded="false" ';
			$input .= ' [aria-expanded]="owpAmpMenuExpanded ? \'true\' : \'false\'" ';
		 ...
		 * @return string
		 */
		public function ctemawp_fullscreen_menu_close( $input ) {
			$input .= ' on="tap:AMP.setState( { owpAmpMenuExpanded: ! owpAmpMenuExpanded } ),ctemawp-body.toggleClass(class=clr)" ';
		 ...
		 * @return string
		 */
		public function ctemawp_attrs_vertical_header_toggle( $input ) {
			$input .= ' on="tap:AMP.setState( { owpAmpVhMenuExpanded: ! owpAmpVhMenuExpanded } ),ctemawp-body.toggleClass(class=amp-vertical-header)" ';
		...
		public function menu_arrow_button_markup( $item_output, $item ) {
			$item_output  = apply_filters( 'ctemawp_toggle_submenu_icon', $item_output, $item );
			$item_output .= '<button ' . owp_attr(
				...
				$item
			) . '><span class="screen-reader-text">' . __( 'Menu Toggle', 'ctemawp' ) . '</span></button>';
		 ...
		 * @return string
		 */
		public function ctemawp_nav_item_search_bar( $input ) {
		 ...
		 * @return string
		 */
		public function ctemawp_menu_search_bar( $input ) {
		 ...
		 * @return string
		 */
		public function ctemawp_menu_overlay_search_bar( $input ) {
		...
		/**
		 * Add AMP attributes to the ctemawp nav wrapper.
		 *
		 * @param string $input the data attrs already existing in search toggle.
		 * @return string
		 */
		public function ctemawp_attrs_nav_wrapper( $input ) {

			if ( 'header_replace' !== get_theme_mod( 'ctema_menu_search_style', 'drop_down' ) ) {
				return;
			}
		 ...
		 */
		public function ctemawp_amp_mini_cart_style( $style ) {
			$style = 'custom_link';
			return $style;
		}
