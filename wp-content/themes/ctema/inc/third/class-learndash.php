...
if ( ! class_exists( 'CtemaWP_LearnDash' ) ) :

	class CtemaWP_LearnDash {
		...
		public static function body_class( $classes ) {
			...
			if( ( is_singular( array( 'sfwd-courses', 'sfwd-lessons', 'sfwd-quiz', 'sfwd-topic' ) ) )
				 && true == get_theme_mod( 'ctema_ld_distraction_free_learning', false ) ) {
		...
		public static function layouts( $class ) {
			if ( is_singular( array( 'sfwd-quiz' ) ) || is_tax( array( 'ld_course_category', 'ld_course_tag', 'ld_lesson_category', 'ld_lesson_tag', 'ld_topic_category', 'ld_topic_tag' ) ) ) {
				$class = get_theme_mod( 'ctema_ld_global_layout', 'full-width' );
			}
			elseif ( is_singular( array( 'sfwd-lessons', 'sfwd-topic' ) ) ) {
				$class = get_theme_mod( 'ctema_ld_lesson_layout', 'left-sidebar' );
			} elseif ( is_singular( 'sfwd-courses' ) ) {
				$class = get_theme_mod( 'ctema_ld_course_layout', 'left-sidebar' );
			}
			return $class;
		}
		...
		public static function bs_class( $class ) {
			if ( is_singular( array( 'sfwd-quiz', 'sfwd-topic' ) ) || is_tax( array( 'ld_course_category', 'ld_course_tag', 'ld_lesson_category', 'ld_lesson_tag', 'ld_topic_category', 'ld_topic_tag' ) ) ) {
				$class = get_theme_mod( 'ctema_ld_global_both_sidebars_style', 'scs-style' );
			}
			elseif ( is_singular( array( 'sfwd-lessons', 'sfwd-topic' ) ) ) {
				$class = get_theme_mod( 'ctema_ld_lesson_both_sidebars_style', 'scs-style' );
			} elseif ( is_singular( 'sfwd-courses' ) ) {
				$class = get_theme_mod( 'ctema_ld_course_both_sidebars_style', 'scs-style' );
			}
			return $class;
		}
		...
		public static function sidebar_order( $order ) {
			if ( is_singular( array( 'sfwd-quiz' ) ) || is_tax( array( 'ld_course_category', 'ld_course_tag', 'ld_lesson_category', 'ld_lesson_tag', 'ld_topic_category', 'ld_topic_tag' ) ) ) {
				$order = get_theme_mod( 'ctema_ld_global_sidebar_order', 'content-sidebar' );
			}
			elseif ( is_singular( array( 'sfwd-lessons', 'sfwd-topic' ) ) ) {
				$order = get_theme_mod( 'ctema_ld_lesson_sidebar_order', 'content-sidebar' );
			} elseif ( is_singular( 'sfwd-courses' ) ) {
				$order = get_theme_mod( 'ctema_ld_course_sidebar_order', 'content-sidebar' );
			}
			return $order;
		}
		...
		public static function add_custom_scripts() {
			...
			wp_enqueue_style( 'ctemawp-learndash', CTEMAWP_CSS_DIR_URI .'learndash/learndash.min.css' );
		}
		...
		public static function typography_settings( $settings ) {
			$settings['learndash_course_title'] = array(
				'label' 				=> esc_html__( 'LearnDash Titles', 'ctemawp' ),
			...
			$settings['learndash_table_heading'] = array(
				'label' 				=> esc_html__( 'LearnDash Table Heading', 'ctemawp' ),
			...
			$settings['learndash_table_item'] = array(
				'label' 				=> esc_html__( 'LearnDash Table Item', 'ctemawp' ),
			...
			$settings['learndash_buy_button'] = array(
				'label'                 => esc_html__( 'LearnDash Take Course Button', 'ctemawp' ),
