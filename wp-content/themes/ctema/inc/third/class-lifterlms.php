...
if ( ! class_exists( 'CtemaWP_LifterLMS' ) ) :

	class CtemaWP_LifterLMS {
		...
		public function __construct() {
			...
			add_action( 'ctema_main_metaboxes_post_types', array( $this, 'add_metabox' ), 20 );
			...
			// Fix for the CtemaWP Settings values not saved
		...
		/**
		 * Add the CtemaWP Settings metabox into the custom post types
		 *
		 * @since 1.2.10
		 */
		...
		public static function body_class( $classes ) {
			...
			if ( is_llms_checkout()
					&& true == get_theme_mod( 'ctema_llms_distraction_free_checkout', false ) ) {
				$classes[] = 'llms-distraction-free';
			}

			if( ( is_course() || is_lesson() ) 
				&& true == get_theme_mod( 'ctema_llms_distraction_free_learning', false ) ) {
				$classes[] = 'llms-distraction-free';	
			}
		...
		public static function layouts( $class ) {
			if ( is_llms_account_page() || is_membership() || is_memberships() || is_quiz() || is_courses() || is_tax( array( 'course_cat', 'course_tag', 'course_difficulty', 'course_track', 'membership_tag', 'membership_cat' ) ) ) {
				$class = get_theme_mod( 'ctema_llms_global_layout', 'full-width' );
			}
			elseif ( is_lesson() ) {
				$class = get_theme_mod( 'ctema_llms_lesson_layout', 'left-sidebar' );
			} elseif ( is_course() ) {
				$class = get_theme_mod( 'ctema_llms_course_layout', 'left-sidebar' );
			}
			return $class;
		}
		...
		public static function bs_class( $class ) {
			if ( is_llms_account_page() || is_membership() || is_memberships() || is_quiz() || is_courses() || is_tax( array( 'course_cat', 'course_tag', 'course_difficulty', 'course_track', 'membership_tag', 'membership_cat' ) ) ) {
				$class = get_theme_mod( 'ctema_llms_global_both_sidebars_style', 'scs-style' );
			}
			elseif ( is_lesson() ) {
				$class = get_theme_mod( 'ctema_llms_lesson_both_sidebars_style', 'scs-style' );
			} elseif ( is_course() ) {
				$class = get_theme_mod( 'ctema_llms_course_both_sidebars_style', 'scs-style' );
			}
			return $class;
		}
		...
		public static function sidebar_order( $order ) {
			if ( is_llms_account_page() || is_membership() || is_memberships() || is_quiz() || is_courses() || is_tax( array( 'course_cat', 'course_tag', 'course_difficulty', 'course_track', 'membership_tag', 'membership_cat' ) ) ) {
				$order = get_theme_mod( 'ctema_llms_global_sidebar_order', 'content-sidebar' );
			}
			elseif ( is_lesson() ) {
				$order = get_theme_mod( 'ctema_llms_lesson_sidebar_order', 'content-sidebar' );
			} elseif ( is_course() ) {
				$order = get_theme_mod( 'ctema_llms_course_sidebar_order', 'content-sidebar' );
			}
			return $order;
		}
		...
		public static function add_custom_scripts() {
			...
			wp_enqueue_style( 'ctemawp-llms', CTEMAWP_CSS_DIR_URI .'llms/llms.min.css' );
		}
		...
		function before_main_content_start() {
			...
			do_action( 'ctema_before_content_wrap' ); ?>
		...
		function course_details(){
			$details = get_theme_mod( 'ctema_llms_course_details', array( 'image', 'description', 'meta', 'author', 'progress', 'syllabus' ) );
		...
		function quiz_settings( $fields ) {
			$fields['quiz']['layout'] = array(
				'title' => esc_html__( 'CtemaWP Theme Setting', 'ctemawp' ),
				'toggleable' => true,
				'fields' => array(
					array(
						array(
							'attribute' => 'ctema_post_layout',
							'label'     => esc_html__( 'Layout', 'ctemawp' ),
							'type'      => 'select',
							'options' => array(
								''              => esc_html__( 'Default', 'ctemawp' ),
								'right-sidebar' => esc_html__( 'Right Sidebar', 'ctemawp' ),
								'left-sidebar'  => esc_html__( 'Left Sidebar', 'ctemawp' ),
								'full-width'    => esc_html__( 'Full Width', 'ctemawp' ),
								'full-screen'   => esc_html__( 'Full Screen', 'ctemawp' ),
								'both-sidebars' => esc_html__( 'Both Sidebar', 'ctemawp' ),
							),
						),
					),
				),
		...
		function archive_columns( $columns ) {
			if ( is_post_type_archive( 'course' ) || is_tax( array( 'course_cat', 'course_tag', 'course_difficulty', 'course_track' ) ) ) {
				$columns = get_theme_mod( 'ctema_llms_courses_columns', 3 );
				$columns = $columns ? $columns : '3';
				return $columns;
			} elseif ( is_post_type_archive( 'llms_membership' ) || is_tax( array( 'membership_tag', 'membership_cat' ) ) ) {
				$columns = get_theme_mod( 'ctema_llms_membership_columns', 3 );
				$columns = $columns ? $columns : '3';
				return $columns;
			}
		}
		...
		function archive_responsive_columns( $classes ) {
			if ( is_post_type_archive( 'course' ) || is_tax( array( 'course_cat', 'course_tag', 'course_difficulty', 'course_track' ) ) ) {
				// Responsive columns
				$tablet_columns    = get_theme_mod( 'ctema_llms_tablet_courses_columns' );
				$mobile_columns    = get_theme_mod( 'ctema_llms_mobile_courses_columns' );
				...
				// Responsive columns
				$tablet_columns    = get_theme_mod( 'ctema_llms_tablet_membership_columns' );
				$mobile_columns    = get_theme_mod( 'ctema_llms_mobile_membership_columns' );
		...
		public static function typography_settings( $settings ) {
			$settings['lifterlms_course_title'] = array(
				'label' 				=> esc_html__( 'LifterLMS Course/Lesson Title', 'ctemawp' ),
			...
			$settings['lifterlms_section_title'] = array(
				'label' 				=> esc_html__( 'LifterLMS Section Title', 'ctemawp' ),
			...
			$settings['lifterlms_lesson_title'] = array(
				'label' 				=> esc_html__( 'LifterLMS Lesson Title', 'ctemawp' ),
			...
			$settings['lifterlms_buy_button'] = array(
				'label'                 => esc_html__( 'LifterLMS Buy Button', 'ctemawp' ),
