...
// Start and run class
if ( ! class_exists( 'CtemaWP_EDD_Config' ) ) {

	class CtemaWP_EDD_Config {
		...
		public function __construct() {
			...
			require_once CTEMAWP_INC_DIR . 'edd/edd-helpers.php';
			...
			if ( ! is_admin() ) {
				...
				add_filter( 'ctema_get_sidebar', array( $this, 'display_edd_sidebar' ) );
			...
			// Add cart overlay
			if ( 'yes' == get_theme_mod( 'ctema_edd_display_cart_download_added', 'no' ) ) {
		...
		public static function body_class( $classes ) {
			...
			if ( edd_is_checkout()
					&& true == get_theme_mod( 'ctema_edd_distraction_free_checkout', false ) ) {
		...
		public static function register_edd_sidebar() {
			...
			if ( ! get_theme_mod( 'ctema_edd_custom_sidebar', true ) ) {
				return;
			}
			...
			register_sidebar(
				array(
					'name'          => esc_html__( 'EDD Sidebar', 'ctemawp' ),
					'id'            => 'edd_sidebar',
		...
		public static function layouts( $class ) {
			if ( is_post_type_archive( 'download' ) ||
				...
				is_tax( 'download_tag' ) ) {
				$class = get_theme_mod( 'ctema_edd_archive_layout', 'left-sidebar' );
			} elseif ( is_singular( 'download' ) ) {
				$class = get_theme_mod( 'ctema_edd_download_layout', 'left-sidebar' );
			}
			return $class;
		}
		...
		public static function bs_class( $class ) {
			if ( is_post_type_archive( 'download' ) ||
				is_tax( 'download_category' ) ||
				is_tax( 'download_tag' ) ) {
				$class = get_theme_mod( 'ctema_edd_archive_both_sidebars_style', 'scs-style' );
			} elseif ( is_singular( 'download' ) ) {
				$class = get_theme_mod( 'ctema_edd_product_both_sidebars_style', 'scs-style' );
			}
			return $class;
		}
		...
		public static function sidebar_order( $order ) {
			if ( is_post_type_archive( 'download' ) ||
				is_tax( 'download_category' ) ||
				is_tax( 'download_tag' ) ) {
				$order = get_theme_mod( 'ctema_edd_archive_sidebar_order', 'content-sidebar' );
			} elseif ( is_singular( 'download' ) ) {
				$order = get_theme_mod( 'ctema_edd_download_sidebar_order', 'content-sidebar' );
			}
			return $order;
		}
		...
		public static function add_custom_scripts() {
			wp_enqueue_style( 'ctemawp-edd', CTEMAWP_CSS_DIR_URI . 'edd/edd.min.css', array(), CTEMAWP_THEME_VERSION, 'all' );
			wp_enqueue_script( 'ctemawp-edd-ajax-cart', CTEMAWP_JS_DIR_URI . 'wp-plugins/easy-digital-downloads/edd-ajax-cart.min.js', array(), CTEMAWP_THEME_VERSION, true );

			// If display cart when product added.
			if ( 'yes' === get_theme_mod( 'ctema_edd_display_cart_download_added', 'no' ) ) {
				wp_enqueue_script( 'ctemawp-edd-display-cart', CTEMAWP_JS_DIR_URI . 'wp-plugins/easy-digital-downloads/edd-display-cart.min.js', array(), CTEMAWP_THEME_VERSION, true );
			}
		}
		...
		public static function add_download_classes( $classes ) {
			...
			// Vars
			$content_alignment = get_theme_mod( 'ctema_edd_archive_entry_content_alignment', 'center' );
			$content_alignment = $content_alignment ? $content_alignment : 'center';

			$classes[] = 'col';
			$classes[] = ctemawp_grid_class( get_theme_mod( 'ctema_edd_archive_columns', 3 ) );
		...
		public static function menu_cart_icon( $items, $args ) {
			...
			// Get style
			$style        = ctemawp_edd_menu_cart_style();
			...
			if ( true == get_theme_mod( 'ctema_edd_menu_icon_hide_if_empty', false )
			...
			if ( 'full_screen' == $header_style ) {
				$items .= '<li class="edd-cart-link"><a href="' . edd_get_checkout_uri() . '">' . esc_html__( 'Your cart', 'ctemawp' ) . '</a></li>';
			} else {
		...
		public static function get_cart_icon() {
			...
			$style        = ctemawp_edd_menu_cart_style();
			$header_style = ctemawp_header_style();
			$cart_style   = get_theme_mod( 'ctema_edd_cart_dropdown_style', 'compact' );
			...
			<li class="<?php echo esc_attr( $classes ); ?>">
				<?php ctemawp_eddmenucart_menu_item(); ?>
				<?php
				if ( 'drop_down' == $style
		...
		public static function typography_settings( $settings ) {
			$settings['edd_product_title'] = array(
				'label'    => esc_html__( 'EDD Product Title', 'ctemawp' ),
			...
			$settings['edd_product_price'] = array(
				'label'    => esc_html__( 'EDD Product Price', 'ctemawp' ),
			...
			$settings['edd_product_add_to_cart'] = array(
				'label'    => esc_html__( 'EDD Product Add To Cart', 'ctemawp' ),
