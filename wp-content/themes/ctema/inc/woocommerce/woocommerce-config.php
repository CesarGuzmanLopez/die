...
// Start and run class.
if ( ! class_exists( 'CtemaWP_WooCommerce_Config' ) ) {

	class CtemaWP_WooCommerce_Config {
		...
		public function __construct() {
			...
			require_once CTEMAWP_INC_DIR . 'woocommerce/woocommerce-helpers.php';
			...
			if ( ! is_admin() ) {
				...
				add_filter( 'ctema_get_sidebar', array( $this, 'display_woo_sidebar' ) );

				// Set correct post layouts.
				add_filter( 'ctema_post_layout_class', array( $this, 'layouts' ) );

				// Set correct both sidebars layout style.
				add_filter( 'ctema_both_sidebars_style', array( $this, 'bs_class' ) );

				// Set correct mobile sidebar order.
				add_filter( 'ctema_sidebar_order', array( $this, 'sidebar_order' ) );
			...
			if ( 'yes' != get_theme_mod( 'ctema_woo_remove_custom_features', 'no' ) ) {
				...
				// Define accents.
				add_filter( 'ctema_primary_texts', array( $this, 'primary_texts' ) );
				add_filter( 'ctema_primary_svg_icons', array( $this, 'primary_svg_icons' ) );
				add_filter( 'ctema_primary_borders', array( $this, 'primary_borders' ) );
				add_filter( 'ctema_primary_backgrounds', array( $this, 'primary_backgrounds' ) );
				add_filter( 'ctema_hover_primary_backgrounds', array( $this, 'hover_primary_backgrounds' ) );
				...
				if ( ! is_admin() ) {
					...
					add_filter( 'ctema_post_subheading', array( $this, 'post_subheading' ) );

					// Show/hide next/prev on products.
					add_filter( 'ctema_has_next_prev', array( $this, 'next_prev' ) );

					// Border colors.
					add_filter( 'ctema_border_color_elements', array( $this, 'border_color_elements' ) );

				}

				// Main Woo Actions.
				add_action( 'wp_enqueue_scripts', array( $this, 'add_custom_scripts' ), 99 );
				add_filter( 'ctema_localize_array', array( $this, 'localize_array' ) );
				if ( get_theme_mod( 'ctema_woo_shop_result_count', true )
					|| get_theme_mod( 'ctema_woo_shop_sort', true )
					|| get_theme_mod( 'ctema_woo_grid_list', true )
					|| true == get_theme_mod( 'ctema_woo_off_canvas_filter', false ) ) {
				...
				add_action( 'woocommerce_before_account_navigation', array( $this, 'ctemawp_before_account_navigation' ) );
				add_action( 'woocommerce_after_account_navigation', array( $this, 'ctemawp_after_account_navigation' ) );
				if ( get_option( 'woocommerce_enable_myaccount_registration' ) !== 'yes' ) {
					add_action( 'woocommerce_before_customer_login_form', array( $this, 'ctemawp_login_wrap_before' ) );
					add_action( 'woocommerce_after_customer_login_form', array( $this, 'ctemawp_login_wrap_after' ) );
				}
				if ( get_theme_mod( 'ctema_woo_category_image', 'no' ) == 'yes' ) {
					add_action( 'woocommerce_archive_description', array( $this, 'woocommerce_category_image' ), 2 );
				}

				// Quick view.
				if ( get_theme_mod( 'ctema_woo_quick_view', true ) ) {
					if ( 'hover' != get_theme_mod( 'ctema_woo_products_style', 'default' ) ) {
						if ( 'gallery-slider' == get_theme_mod( 'ctema_woo_product_entry_style', 'image-swap' ) ) {
							add_action( 'ctema_after_product_entry_slider', array( $this, 'quick_view_button' ) );
						} else {
							add_action( 'ctema_after_product_entry_image', array( $this, 'quick_view_button' ) );
						}
					}
					add_action( 'wp_ajax_ctemawp_product_quick_view', array( $this, 'product_quick_view_ajax' ) );
					add_action( 'wp_ajax_nopriv_ctemawp_product_quick_view', array( $this, 'product_quick_view_ajax' ) );
					add_action( 'wp_footer', array( $this, 'quick_view_template' ) );
					add_action( 'ctema_woo_quick_view_product_image', 'woocommerce_show_product_sale_flash', 10 );
					add_action( 'ctema_woo_quick_view_product_image', array( $this, 'quick_view_image' ), 20 );
					add_action( 'ctema_woo_quick_view_product_content', array( $this, 'single_product_content' ), 10 );
				}

				// Ajax single product add to cart.
				add_action( 'wp_ajax_ctemawp_add_cart_single_product', array( $this, 'add_cart_single_product_ajax' ) );
				add_action( 'wp_ajax_nopriv_ctemawp_add_cart_single_product', array( $this, 'add_cart_single_product_ajax' ) );

				// Add cart overlay
				if ( 'yes' == get_theme_mod( 'ctema_woo_display_cart_product_added', 'no' ) ) {
					add_action( 'ctema_after_footer', array( $this, 'cart_overlay' ), 99 );
				}

				// Add mobile menu mini cart.
				if ( get_theme_mod( 'ctema_woo_add_mobile_mini_cart', true ) ) {
				...
				if ( true == get_theme_mod( 'ctema_woocommerce_display_navigation', true ) ) {
				...
				if ( 'on' == get_theme_mod( 'ctema_woo_display_floating_bar', 'on' ) ) {
					add_action( 'ctema_before_main', array( $this, 'floating_bar' ) );

					// Ajax add to cart.
					add_action( 'wp_ajax_ctemawp_add_cart_floating_bar', array( $this, 'add_cart_floating_bar_ajax' ) );
					add_action( 'wp_ajax_nopriv_ctemawp_add_cart_floating_bar', array( $this, 'add_cart_floating_bar_ajax' ) );
				}
				...
				// Sale badge content.
				if ( 'percent' == get_theme_mod( 'ctema_woo_sale_badge_content', 'sale' ) ) {
				...
				add_filter( 'ctema_display_top_bar', array( $this, 'distraction_free' ), 11 );
				add_filter( 'ctema_display_navigation', array( $this, 'distraction_free' ), 11 );
				...
				add_filter( 'ctema_display_page_header', array( $this, 'distraction_free' ), 11 );
				add_filter( 'ctema_display_footer_widgets', array( $this, 'distraction_free' ), 11 );
				add_filter( 'ctema_display_scroll_up_button', array( $this, 'distraction_free' ), 11 );
				...
				if ( true == get_theme_mod( 'ctema_woo_multi_step_checkout', false ) ) {
					...
					// Checkout validation.
					add_action( 'wp_ajax_ctemawp_validate_checkout', array( $this, 'validate_checkout_callback' ) );
					add_action( 'wp_ajax_nopriv_ctemawp_validate_checkout', array( $this, 'validate_checkout_callback' ) );
					...
					add_action( 'ctema_woocommerce_checkout_order_review', 'woocommerce_order_review', 10 );
					add_action( 'ctema_woocommerce_checkout_payment', 'woocommerce_checkout_payment', 20 );
					add_action( 'ctema_checkout_login_form', array( $this, 'checkout_login_form' ), 10 );
					add_action( 'ctema_woocommerce_checkout_coupon', 'woocommerce_checkout_coupon_form', 10 );
				...
				add_filter( 'ctema_typography_settings', array( $this, 'typography_settings' ) );
		...
		public static function woo_section( $wp_customize ) {
			$wp_customize->get_section( 'woocommerce_checkout' )->panel                                      = 'ctema_woocommerce_panel';
			$wp_customize->get_section( 'woocommerce_store_notice' )->panel                                  = 'ctema_woocommerce_panel';
			$wp_customize->get_section( 'woocommerce_product_images' )->panel                                = 'ctema_woocommerce_panel';
			$wp_customize->get_section( 'woocommerce_product_images' )->priority                             = 999;
			$wp_customize->get_control( 'woocommerce_shop_page_display' )->section                           = 'ctema_woocommerce_archives';
			$wp_customize->get_control( 'woocommerce_category_archive_display' )->section                    = 'ctema_woocommerce_archives';
			$wp_customize->get_control( 'woocommerce_default_catalog_orderby' )->section                     = 'ctema_woocommerce_archives';
			$wp_customize->get_control( 'woocommerce_checkout_company_field' )->section                      = 'ctema_woocommerce_checkout';
			$wp_customize->get_control( 'woocommerce_checkout_address_2_field' )->section                    = 'ctema_woocommerce_checkout';
			$wp_customize->get_control( 'woocommerce_checkout_phone_field' )->section                        = 'ctema_woocommerce_checkout';
			$wp_customize->get_control( 'woocommerce_checkout_highlight_required_fields' )->section          = 'ctema_woocommerce_checkout';

			if ( current_user_can( 'manage_privacy_options' ) ) {
				$wp_customize->get_control( 'wp_page_for_privacy_policy' )->section                          = 'ctema_woocommerce_checkout';
			}
			$wp_customize->get_control( 'woocommerce_terms_page_id' )->section                               = 'ctema_woocommerce_checkout';
			$wp_customize->get_control( 'woocommerce_checkout_privacy_policy_text' )->section                = 'ctema_woocommerce_checkout';
			$wp_customize->get_control( 'woocommerce_checkout_terms_and_conditions_checkbox_text' )->section = 'ctema_woocommerce_checkout';
		}
		...
		public static function register_woo_sidebar() {
			...
			if ( ! get_theme_mod( 'ctema_woo_custom_sidebar', true ) ) {
				return;
			}
			...
			register_sidebar(
				array(
					'name'          => esc_html__( 'WooCommerce Sidebar', 'ctemawp' ),
					'id'            => 'woo_sidebar',
		...
		public static function layouts( $class ) {
			if ( ctemawp_is_woo_shop()
				|| ctemawp_is_woo_tax() ) {
				$class = get_theme_mod( 'ctema_woo_shop_layout', 'left-sidebar' );
			} elseif ( ctemawp_is_woo_single() ) {
				$class = get_theme_mod( 'ctema_woo_product_layout', 'left-sidebar' );
			}
			return $class;
		}
		...
		public static function bs_class( $class ) {
			if ( ctemawp_is_woo_shop()
				|| ctemawp_is_woo_tax() ) {
				$class = get_theme_mod( 'ctema_woo_shop_both_sidebars_style', 'scs-style' );
			} elseif ( ctemawp_is_woo_single() ) {
				$class = get_theme_mod( 'ctema_woo_product_both_sidebars_style', 'scs-style' );
			}
			return $class;
		}
		...
		public static function sidebar_order( $order ) {
			if ( ctemawp_is_woo_shop()
				|| ctemawp_is_woo_tax() ) {
				$order = get_theme_mod( 'ctema_woo_shop_sidebar_order', 'content-sidebar' );
			} elseif ( ctemawp_is_woo_single() ) {
				$order = get_theme_mod( 'ctema_woo_product_sidebar_order', 'content-sidebar' );
			}
			return $order;
		}
		...
		public static function mini_cart_style() {
			...
			wp_enqueue_style( 'ctemawp-woo-mini-cart', CTEMAWP_CSS_DIR_URI . 'woo/woo-mini-cart.min.css' );

			// If rtl.
			if ( is_RTL() ) {
				wp_enqueue_style( 'ctemawp-woo-mini-cart-rtl', CTEMAWP_CSS_DIR_URI . 'woo/woo-mini-cart-rtl.css' );
			}
		...
		public function init() {
			...
			if ( 'on' != get_theme_mod( 'ctema_woocommerce_display_related_items', 'on' ) ) {
				remove_action( 'woocommerce_after_single_product_summary', 'woocommerce_output_related_products', 20 );
			}

			// Remove orderby if disabled.
			if ( ! get_theme_mod( 'ctema_woo_shop_sort', true ) ) {
			...
			if ( class_exists( 'WooCommerce_Germanized' ) ) {
				...
				add_action( 'ctema_after_archive_product_inner', array( $this, 'woocommerce_germanized' ) );
				...
				add_action( 'ctema_after_single_product_price', array( $this, 'woocommerce_germanized_single' ), 12 );
				...
				add_action( 'ctema_after_single_product_excerpt', array( $this, 'woocommerce_germanized_single_meta' ), 12 );
			}

			if ( class_exists( 'YITH_WCWL' ) ) {

				add_action( 'wp_enqueue_scripts', 'ctema_dequeue_yith_wl_scripts' );
		...
		public function add_action_to_multi_currency_ajax( $ajax_actions ) {
			$ajax_actions[] = 'ctemawp_product_quick_view';
			return $ajax_actions;
		}
		...
		public function shop_pagination() {
			if ( 'infinite_scroll' == get_theme_mod( 'ctema_woo_pagination_style', 'standard' ) && is_woocommerce() ) {
		...
		public static function infinite_pagination() {
			...
			// Last text.
			$last = get_theme_mod( 'ctema_woo_infinite_scroll_last_text' );
			$last = ctemawp_tm_translation( 'ctema_woo_infinite_scroll_last_text', $last );
			$last = $last ? $last : esc_html__( 'End of content', 'ctemawp' );

			// Error text.
			$error = get_theme_mod( 'ctema_woo_infinite_scroll_error_text' );
			$error = ctemawp_tm_translation( 'ctema_woo_infinite_scroll_error_text', $error );
			...
			<div class="infinite-scroll-nav clr">
				<div class="alignleft newer-posts"><?php echo get_previous_posts_link( '&larr; ' . esc_html__( 'Newer Posts', 'ctemawp' ) ); ?></div>
				<div class="alignright older-posts"><?php echo get_next_posts_link( esc_html__( 'Older Posts', 'ctemawp' ) . ' &rarr;', $wp_query->max_num_pages ); ?></div>
			</div>
			<?php
		}
		...
		public static function body_class( $classes ) {
			...
			if ( 'dropdown' == get_theme_mod( 'ctema_woo_cat_widget_style', 'default' ) ) {
				$classes[] = 'woo-dropdown-cat';
			}

			// Distraction free class.
			if ( ( is_cart()
					&& true == get_theme_mod( 'ctema_woo_distraction_free_cart', false ) )
				|| ( is_checkout()
					&& true == get_theme_mod( 'ctema_woo_distraction_free_checkout', false ) ) ) {
				$classes[] = 'distraction-free';
			}

			// My Account page style.
			$style = get_theme_mod( 'ctema_woo_account_page_style', 'original' );
			if ( 'side' == $style ) {
		...
		public static function add_custom_scripts() {
			...
			$active_status = true;
			if ( class_exists( 'Ctema_eCommerce' ) ) {
				if ( function_exists( 'oec_get_theme_version' ) ) {
					$active_status = oec_get_theme_version();
				}
			}

			if ( ! class_exists( 'Ctema_eCommerce' ) || true === $active_status ) {

				// Register WooCommerce styles
				wp_enqueue_style( 'ctemawp-woocommerce', CTEMAWP_CSS_DIR_URI . 'woo/woocommerce.min.css' );
				wp_enqueue_style( 'ctemawp-woo-star-font', CTEMAWP_CSS_DIR_URI . 'woo/woo-star-font.min.css' );

				// If rtl
				if ( is_RTL() ) {
					wp_enqueue_style( 'ctemawp-woocommerce-rtl', CTEMAWP_CSS_DIR_URI . 'woo/woocommerce-rtl.css' );
				}

				// If products hover style
				if ( 'hover' == get_theme_mod( 'ctema_woo_products_style', 'default' ) ) {
					wp_enqueue_style( 'ctemawp-woo-hover-style', CTEMAWP_CSS_DIR_URI . 'woo/hover-style.min.css' );
				}

				// If quick view.
				if ( get_theme_mod( 'ctema_woo_quick_view', true ) ) {
					wp_enqueue_style( 'ctemawp-woo-quick-view', CTEMAWP_CSS_DIR_URI . 'woo/woo-quick-view.min.css' );
				}

				// If whislist.
				if ( class_exists( 'TInvWL_Wishlist' ) || class_exists( 'YITH_WCWL' ) ) {
					wp_enqueue_style( 'ctemawp-wishlist', CTEMAWP_CSS_DIR_URI . 'woo/wishlist.min.css' );
				}

				// If floating bar
				if ( 'on' == get_theme_mod( 'ctema_woo_display_floating_bar', 'on' )
					&& ctemawp_is_woo_single() ) {
					wp_enqueue_style( 'ctemawp-woo-floating-bar', CTEMAWP_CSS_DIR_URI . 'woo/woo-floating-bar.min.css' );
				}

				// If multi step checkout
				if ( true == get_theme_mod( 'ctema_woo_multi_step_checkout', false )
					&& is_checkout() ) {
					wp_enqueue_style( 'ctemawp-woo-multistep-checkout', CTEMAWP_CSS_DIR_URI . 'woo/woo-multistep-checkout.min.css' );
				}
			}

			// If products hover style
			if ( 'hover' == get_theme_mod( 'ctema_woo_products_style', 'default' ) ) {
				wp_enqueue_script( 'ctemawp-woo-hover-style', CTEMAWP_JS_DIR_URI . 'wp-plugins/woocommerce/woo-hover-style.min.js', array( 'ctemawp-main' ), CTEMAWP_THEME_VERSION, true );
			}

			// If dropdown category widget style
			if ( 'dropdown' == get_theme_mod( 'ctema_woo_cat_widget_style', 'default' ) ) {
				wp_enqueue_script( 'ctemawp-woo-cat-widget', CTEMAWP_JS_DIR_URI . 'wp-plugins/woocommerce/woo-cat-widget.min.js', array( 'ctemawp-main' ), CTEMAWP_THEME_VERSION, true );
			}

			// If vertical thumbnails style
			if ( 'vertical' == get_theme_mod( 'ctema_woo_product_thumbs_layout', 'horizontal' ) ) {
				wp_enqueue_script( 'ctemawp-woo-thumbnails', CTEMAWP_JS_DIR_URI . 'wp-plugins/woocommerce/woo-thumbnails.min.js', array( 'ctemawp-main' ), CTEMAWP_THEME_VERSION, true );
			}

			// If quick view.
			if ( get_theme_mod( 'ctema_woo_quick_view', true ) ) {
				wp_enqueue_script( 'ctemawp-woo-quick-view', CTEMAWP_JS_DIR_URI . 'wp-plugins/woocommerce/woo-quick-view.min.js', array( 'ctemawp-main' ), CTEMAWP_THEME_VERSION, true );
				wp_enqueue_script( 'wc-add-to-cart-variation' );
				wp_enqueue_script( 'flexslider' );
			}

			// If single product ajax add to cart
			if ( true == get_theme_mod( 'ctema_woo_product_ajax_add_to_cart', false )
				&& ctemawp_is_woo_single() ) {
				wp_enqueue_script( 'ctemawp-woo-ajax-addtocart', CTEMAWP_JS_DIR_URI . 'wp-plugins/woocommerce/woo-ajax-add-to-cart.min.js', array( 'ctemawp-main' ), CTEMAWP_THEME_VERSION, true );
			}

			// If floating bar
			if ( 'on' == get_theme_mod( 'ctema_woo_display_floating_bar', 'on' )
				&& ctemawp_is_woo_single() ) {
				wp_enqueue_script( 'ctemawp-woo-floating-bar', CTEMAWP_JS_DIR_URI . 'wp-plugins/woocommerce/woo-floating-bar.min.js', array( 'ctemawp-main' ), CTEMAWP_THEME_VERSION, true );
			}

			// If display cart when product added
			if ( 'yes' == get_theme_mod( 'ctema_woo_display_cart_product_added', 'no' ) ) {
				wp_enqueue_script( 'ctemawp-woo-display-cart', CTEMAWP_JS_DIR_URI . 'wp-plugins/woocommerce/woo-display-cart.min.js', array( 'ctemawp-main' ), CTEMAWP_THEME_VERSION, true );
			}
			...
			if ( true == get_theme_mod( 'ctema_woo_off_canvas_filter', false )
				&& ( ctemawp_is_woo_shop()
					|| ctemawp_is_woo_tax() ) ) {
				wp_enqueue_script( 'ctemawp-woo-off-canvas', CTEMAWP_JS_DIR_URI . 'wp-plugins/woocommerce/woo-off-canvas.min.js', array( 'ctemawp-main' ), CTEMAWP_THEME_VERSION, true );
			}

			// If mobile menu mini cart
			if ( get_theme_mod( 'ctema_woo_add_mobile_mini_cart', true ) ) {
				wp_enqueue_script( 'ctemawp-woo-mini-cart', CTEMAWP_JS_DIR_URI . 'wp-plugins/woocommerce/woo-mini-cart.min.js', array( 'ctemawp-main' ), CTEMAWP_THEME_VERSION, true );
			}
			...
			if ( true == get_theme_mod( 'ctema_woo_multi_step_checkout', false )
				...
				wp_enqueue_script( 'ctemawp-woo-multistep-checkout', CTEMAWP_JS_DIR_URI . 'wp-plugins/woocommerce/woo-multi-step-checkout.min.js', $woo_deps, CTEMAWP_THEME_VERSION, true );
			}
		...
		public static function localize_array( $array ) {
			...
			if ( true == get_theme_mod( 'ctema_woo_product_ajax_add_to_cart', false ) ) {
				...
				$array['cart_url'] = apply_filters( 'ctema_woocommerce_add_to_cart_redirect', wc_get_cart_url() );
			}
			...
			if ( true == get_theme_mod( 'ctema_woo_multi_step_checkout', false ) ) {
				...
				$array['no_account_btn']         = esc_html__( 'I don&rsquo;t have an account', 'ctemawp' );
				$array['next']                   = esc_html__( 'Next', 'ctemawp' );
			}

			// If quick view, ajax add to cart or floating bar
			if ( get_theme_mod( 'ctema_woo_quick_view', true )
				|| true == get_theme_mod( 'ctema_woo_product_ajax_add_to_cart', false )
				|| 'on' == get_theme_mod( 'ctema_woo_display_floating_bar', 'on' ) ) {
				$array['ajax_url']                = admin_url( 'admin-ajax.php' );
			...
			$array['view_cart'] = esc_html__( 'View cart', 'ctemawp' );

			// Check if the floating bar is enabled for the quantity button
			$array['floating_bar'] = get_theme_mod( 'ctema_woo_display_floating_bar', 'on' );

			// Grouped product button text in the quick view
			$array['grouped_text'] = esc_html__( 'View products', 'ctemawp' );

			$array['multistep_checkout_error'] = __( 'Some required fields are empty. Please fill the required fields to go to the next step.', 'ctemawp' );
		...
		public static function get_mini_cart_sidebar() {
			...
			// Style
			$cart_style = get_theme_mod( 'ctema_woo_cart_dropdown_style', 'compact' );

			// Define classes
			$classes = array( 'ctemawp-cart-sidebar' );
			...
			echo '<div id="ctemawp-cart-sidebar-wrap">';
				echo '<div class="' . $classes . '">';
					echo '<a href="#" class="ctemawp-cart-close">×</a>';
					echo '<p class="owp-cart-title">' . esc_html__( 'Cart', 'ctemawp' ) . '</p><div class="divider"></div>';
					echo '<div class="owp-mini-cart">';
						the_widget( 'WC_Widget_Cart', 'title=' );
					echo '</div>';
				echo '</div>';
				echo '<div class="ctemawp-cart-sidebar-overlay"></div>';
			echo '</div>';

		}

		/**
		 * Adds an opening div "ctemawp-toolbar" around top elements.
		 *
		 * @since 1.1.1
		 */
		public static function add_shop_loop_div() {
			echo '<div class="ctemawp-toolbar clr">';
		}
		...
		public static function register_off_canvas_sidebar() {
			...
			register_sidebar(
				array(
					'name'          => esc_html__( 'Off-Canvas Filters', 'ctemawp' ),
					'description'   => esc_html__( 'Widgets in this area are used in the off canvas sidebar. To enable the Off Canvas filter, go to the WooCommerce > Archives section of the customizer and enable the Display Filter Button option.', 'ctemawp' ),
		...
		public static function off_canvas_filter_button() {
			...
			// Get filter text
			$text = get_theme_mod( 'ctema_woo_off_canvas_filter_text' );
			$text = ctemawp_tm_translation( 'ctema_woo_off_canvas_filter_text', $text );
			$text = $text ? $text : esc_html__( 'Filter', 'ctemawp' );

			$output = '<a href="#" class="ctemawp-off-canvas-filter">' . ctemawp_icon( 'menu', false ) . '<span class="off-canvas-filter-text">' . esc_html( $text ) . '</span></a>';

			echo apply_filters( 'ctemawp_off_canvas_filter_button_output', $output );
		}
		...
		public static function grid_list_buttons() {
			...
			// Titles
			$grid_view = esc_html__( 'Grid view', 'ctemawp' );
			$list_view = esc_html__( 'List view', 'ctemawp' );

			// Active class
			if ( 'list' == get_theme_mod( 'ctema_woo_catalog_view', 'grid' ) ) {
				$list = 'active ';
				$grid = '';
			} else {
				$grid = 'active ';
			...
			$grid_view_icon = ctemawp_icon( 'grid', false );
			$list_view_icon = ctemawp_icon( 'list', false );

			$output = '<nav class="ctemawp-grid-list"><a href="#" id="ctemawp-grid" title="' . esc_html( $grid_view ) . '" class="' . esc_attr( $grid ) . ' grid-btn">' . $grid_view_icon . '</a><a href="#" id="ctemawp-list" title="' . esc_html( $list_view ) . '" class="' . esc_attr( $list ) . ' list-btn">' . $list_view_icon . '</a></nav>';

			echo apply_filters( 'ctemawp_grid_list_buttons_output', $output );
		}

		/**
		 * Closes the opening div "ctemawp-toolbar" around top elements.
		 *
		 * @since 1.1.1
		 */
		...
		public static function loop_shop_per_page() {
			if ( get_theme_mod( 'ctema_woo_shop_result_count', true ) ) {
				$posts_per_page = ( isset( $_GET['products-per-page'] ) ) ? sanitize_text_field( wp_unslash( $_GET['products-per-page'] ) ) : get_theme_mod( 'ctema_woo_shop_posts_per_page', '12' );

				if ( $posts_per_page == 'all' ) {
					$posts_per_page = wp_count_posts( 'product' )->publish;
				}
			} else {
				$posts_per_page = get_theme_mod( 'ctema_woo_shop_posts_per_page' );
		...
		public static function loop_shop_columns() {
			$columns = get_theme_mod( 'ctema_woocommerce_shop_columns', '3' );
		...
		public static function upsell_display() {
			...
			// Get count
			$count = get_theme_mod( 'ctema_woocommerce_upsells_count', '3' );
			$count = $count ? $count : '3';

			// Get columns
			$columns = get_theme_mod( 'ctema_woocommerce_upsells_columns', '3' );
			$columns = $columns ? $columns : '3';
		...
		public static function cross_sell_display() {
			...
			// Get count
			$count = get_theme_mod( 'ctema_woocommerce_cross_sells_count', '2' );
			$count = $count ? $count : '2';

			// Get columns
			$columns = get_theme_mod( 'ctema_woocommerce_cross_sells_columns', '2' );
		...
		public static function related_product_args() {
			...
			// Get posts per page
			$posts_per_page = get_theme_mod( 'ctema_woocommerce_related_count', '3' );
			$posts_per_page = $posts_per_page ? $posts_per_page : '3';

			// Get columns
			$columns = get_theme_mod( 'ctema_woocommerce_related_columns', '3' );
			$columns = $columns ? $columns : '3';
		...
		public static function add_out_of_stock_badge() {
			if ( function_exists( 'ctemawp_woo_product_instock' ) && ! ctemawp_woo_product_instock() ) {
				$label = esc_html__( 'Out of Stock', 'ctemawp' );
				?>
				<div class="outofstock-badge">
					<?php echo esc_html( apply_filters( 'ctema_woo_outofstock_text', $label ) ); ?>
		...
		public static function loop_product_thumbnail() {
			if ( function_exists( 'wc_get_template' ) ) {
				...
				$style = get_theme_mod( 'ctema_woo_product_entry_style' );
		...
		public static function quick_view_button() {
			...
			$button = '<a href="#" id="product_id_' . $product->get_id() . '" class="owp-quick-view" data-product_id="' . $product->get_id() . '" aria-label="' . esc_attr__( 'Quickly preview product', 'ctemawp' ) . ' ' . $product->get_name() . '">' . ctemawp_icon( 'eye', false ) . ' ' . esc_html( ctemawp_theme_strings( 'owp-string-woo-quick-view-text', false ) ) . '</a>';

			echo apply_filters( 'ctema_woo_quick_view_button_html', $button ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
		}
		...
		public static function product_quick_view_ajax() {
			if ( ! isset( $_POST['nonce'] ) || ! wp_verify_nonce( $_POST['nonce'], 'ctemawp' ) ) {
				wp_die();
			}
		...
		public static function product_next_prev_nav() {
			...
			$ctema_next_prev_prod_content = '';

			if ( is_a( $prev_post, 'WP_Post' ) ) {
				$next_arrow = is_rtl() ? ctemawp_icon( 'angle_right', false ) : ctemawp_icon( 'angle_left', false );
				ob_start();
				?>

				<li class="prev-li">
				<a href="<?php echo esc_url( get_the_permalink( $prev_post->ID ) ); ?>" class="owp-nav-link prev" rel="next" aria-label="<?php echo esc_attr( ctemawp_theme_strings( 'owp-string-wai-prev-product', false ) ); ?>"><?php echo $next_arrow; ?></a>
					<a href="<?php echo esc_url( get_the_permalink( $prev_post->ID ) ); ?>" class="owp-nav-text prev-text"><?php echo esc_html( ctemawp_theme_strings( 'owp-string-woo-nav-prev-product', false ) ); ?></a>
				...
				<?php
				$ctema_next_prev_prod_content .= ob_get_clean();
			}

			if ( is_a( $next_post, 'WP_Post' ) ) {
				$prev_arrow = is_rtl() ? ctemawp_icon( 'angle_left', false ) : ctemawp_icon( 'angle_right', false );
				ob_start();
				?>

				<li class="next-li">
					<a href="<?php echo esc_url( get_the_permalink( $next_post->ID ) ); ?>" class="owp-nav-text next-text"><?php echo esc_html( ctemawp_theme_strings( 'owp-string-woo-nav-next-product', false ) ); ?></a>
					<a href="<?php echo esc_url( get_the_permalink( $next_post->ID ) ); ?>" class="owp-nav-link next" rel="next" aria-label="<?php echo esc_attr( ctemawp_theme_strings( 'owp-string-wai-next-product', false ) ); ?>"><?php echo $prev_arrow; ?></i></a>
					<div class="owp-nav-thumb">
			...
			<div class="owp-product-nav-wrap clr">
				<ul class="owp-product-nav">
					<?php echo $ctema_next_prev_prod_content; // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped above. ?>
				</ul>
			</div>
		...
		public static function floating_bar() {
			...
			$fb_woo_cond = get_theme_mod( 'ctema_single_conditional', false );
			...
			$fb_h_tag = 'h2';
			$fb_h_tag = apply_filters( 'ctema_floating_bar_h_tag', $fb_h_tag );
			...
			<div class="owp-floating-bar">
				<div class="container clr">
					<div class="left">
						<p class="selected"><?php echo esc_html( ctemawp_theme_strings( 'owp-string-woo-floating-bar-selected', false ) ); ?></p>
					...
					<?php } ?>
						...
						if ( 'outofstock' == $product->get_stock_status() ) {
							?>
							<p class="stock out-of-stock"><?php echo esc_html( ctemawp_theme_strings( 'owp-string-woo-floating-bar-out-stock', false ) ); ?></p>
							<?php
							...
							if ( $product && $product->is_type( 'simple' ) && $product->is_purchasable() && $product->is_in_stock() && ! $product->is_sold_individually() ) {
								...
								?>
								<button type="submit" class="button top"><?php echo esc_html( ctemawp_theme_strings( 'owp-string-woo-floating-bar-select-btn', false ) ); ?></button>
								<?php
							}
						} else {

							// Get price message display state.
							$fb_woo_cond_msg = get_theme_mod( 'ctema_woo_single_cond_msg', 'yes' );

							if ( 'yes' === $fb_woo_cond_msg ) {

								// Get Add to Cart button replacement message.
								$fb_woo_msg_txt = get_theme_mod( 'ctema_woo_single_cond_msg_text' );
								$fb_woo_msg_txt = $fb_woo_msg_txt ? $fb_woo_msg_txt : esc_html__( 'Log in to view price and purchase', 'ctemawp' );
								?>
							<div class="right">
		 ...
		 * @since 1.0.0
		 */
		public static function ctemawp_before_account_navigation() {
			...
			if ( $current_user->display_name ) {
				...
				$name = esc_html__( 'Welcome!', 'ctemawp' );
			}
			$name = apply_filters( 'ctema_user_profile_name_text', $name );

			echo '<div class="woocommerce-MyAccount-tabs clr">';
				echo '<div class="ctemawp-user-profile clr">';
					echo '<div class="image">' . get_avatar( $current_user->user_email, 128 ) . '</div>';
					echo '<div class="user-info">';
						echo '<p class="name">' . esc_attr( $name ) . '</p>';
						echo '<a class="logout" href="' . esc_url( wp_logout_url( get_permalink() ) ) . '">' . esc_html__( 'Logout', 'ctemawp' ) . '</a>';
					echo '</div>';
				echo '</div>';
		 ...
		 * @since 1.0.0
		 */
		public static function ctemawp_after_account_navigation() {
			echo '</div>';
		}
		...
		public static function add_category_description( $category ) {
			...
			$length      = get_theme_mod( 'ctema_woo_list_excerpt_length', '60' );
		 ...
		 * @since 1.0.0
		 */
		public static function ctemawp_login_wrap_before() {
			echo '<div class="ctemawp-loginform-wrap">';
		}
		 ...
		 * @since 1.0.0
		 */
		public static function ctemawp_login_wrap_after() {
			echo '</div>';
		}
		...
		public static function pagination_args( $args ) {
			...
			// Nav icons.
			$p_arrow = is_rtl() ? ctemawp_icon( 'angle_right', false ) : ctemawp_icon( 'angle_left', false );
			$n_arrow = is_rtl() ? ctemawp_icon( 'angle_left', false ) : ctemawp_icon( 'angle_right', false );
		...
		public static function add_product_classes( $classes ) {
			...
			// Vars
			$product           = wc_get_product( get_the_ID() );
			$content_alignment = get_theme_mod( 'ctema_woo_product_entry_content_alignment', 'center' );
			$content_alignment = $content_alignment ? $content_alignment : 'center';
			$thumbs_layout     = get_theme_mod( 'ctema_woo_product_thumbs_layout', 'horizontal' );
			$thumbs_layout     = $thumbs_layout ? $thumbs_layout : 'horizontal';
			$tabs_layout       = get_theme_mod( 'ctema_woo_product_tabs_layout', 'horizontal' );
			$tabs_layout       = $tabs_layout ? $tabs_layout : 'horizontal';
			$btn_style         = get_theme_mod( 'ctema_woo_product_addtocart_style', 'normal' );
			...
			if ( $product && ! empty( $woocommerce_loop['columns'] ) ) {
				...
				// Column Alignment.
				$classes[] = 'col';
				$classes[] = ctemawp_grid_class( $woocommerce_loop['columns'] );
			...
			// Sale badge style
			$sale_style = get_theme_mod( 'ctema_woo_sale_badge_style', 'square' );
		...
		public static function primary_texts( $texts ) {
			return array_merge(
				array(
					...
					'.woocommerce .ctemawp-grid-list a.active',
					'.woocommerce .ctemawp-grid-list a:hover',
					'.woocommerce .ctemawp-off-canvas-filter:hover',
		...
		public static function primary_svg_icons( $svg_icons ) {
			return array_merge(
				array(
					'.woocommerce .ctemawp-grid-list a.active .owp-icon use',
					'.woocommerce .ctemawp-grid-list a:hover .owp-icon use',
				),
				$svg_icons
			);
		}
		...
		public static function menu_wishlist_icon( $items, $args ) {
			...
			if ( CTEMAWP_ELEMENTOR_ACTIVE
				&& \Elementor\Plugin::$instance->editor->is_edit_mode() ) {
				return $items;
			}

			// Return.
			if ( ! ctema_woo_wishlist()
				|| true != get_theme_mod( 'ctema_woo_wishlist_icon', false )
				|| 'main_menu' != $args->theme_location ) {
				return $items;
			}

			$wl_plugin = get_theme_mod( 'ctema_woo_wl_plugin', 'ti_wl' );
			...
			if ( 'ti_wl' === $wl_plugin ) {
				...
				$woo_wl_count = ctema_woo_wishlist_count();

				$items .= '<a href="' . esc_url( $wishlist_url ) . '">' . ctemawp_icon( 'heart', false ) . '<span class="count">' . esc_html( $woo_wl_count ) . '</i></span></a>';
			}
		...
		public static function menu_cart_icon( $items, $args ) {
			...
			if ( 'full_screen' == $header_style ) {
				$items .= '<li class="woo-cart-link"><a href="' . esc_url( wc_get_cart_url() ) . '">' . esc_html__( 'Your cart', 'ctemawp' ) . '</a></li>';
			} else {
		...
		public static function get_cart_icon() {
			...
			<li class="<?php echo esc_attr( $classes ); ?>">
				<?php ctemawp_wcmenucart_menu_item(); ?>
				<?php
				if ( 'drop_down' == $style
		...
		public static function menu_cart_icon_fragments( $fragments ) {
			ob_start();
			ctemawp_wcmenucart_menu_item();
			$fragments['li.woo-menu-icon a.wcmenucart, .ctemawp-mobile-menu-icon a.wcmenucart'] = ob_get_clean();
		...
		public static function sale_flash() {
			...
			if ( $product->is_type( 'simple' ) || $product->is_type( 'external' ) ) {
				...
				$percent = sprintf( __( '%s', 'ctemawp' ), $maximumper );

			} else {

				$percent = '<span class="onsale">' . __( 'Sale!', 'ctemawp' ) . '</span>';
				return $percent;
		...
		public static function login_register_links() {
			...
			if ( 'original' != get_theme_mod( 'ctema_woo_account_page_style', 'original' ) ) {
				return;
			}
			...
			if ( 'yes' == $registration ) {
				...
				// If registration
			if ( 'yes' == $registration ) {
				$html     .= '<li class="or">' . esc_html__( 'Or', 'ctemawp' ) . '</li>';
				$html     .= '<li class="register">';
					$html .= '<a href="#" class="owp-account-link">' . esc_html__( 'Register', 'ctemawp' ) . '</a>';
				$html     .= '</li>';
			}
				...
				foreach ( $fieldset as $key => $field ) {
					...
					switch ( $fieldset_key ) {
						case 'shipping':
							...
							$field_label = sprintf( __( 'Shipping %s', 'ctemawp' ), $field_label );
							break;
						case 'billing':
							/* translators: %s: field name */
							$field_label = sprintf( __( 'Billing %s', 'ctemawp' ), $field_label );
							break;
					}
					...
					if ( in_array( 'postcode', $format ) ) {
						...
						if ( '' !== $data[ $key ] && ! WC_Validation::is_postcode( $data[ $key ], $country ) ) {
							$errors->add( 'validation', sprintf( __( '%s is not a valid postcode / ZIP.', 'ctemawp' ), '<strong>' . esc_html( $field_label ) . '</strong>' ) );
						}
					}
					...
					if ( in_array( 'phone', $format ) ) {
						...
						if ( '' !== $data[ $key ] && ! WC_Validation::is_phone( $data[ $key ] ) ) {
							...
							$errors->add( 'validation', sprintf( __( '%s is not a valid phone number.', 'ctemawp' ), '<strong>' . esc_html( $field_label ) . '</strong>' ) );
						}
					}
					...
					if ( in_array( 'email', $format ) && '' !== $data[ $key ] ) {
						...
						if ( ! is_email( $data[ $key ] ) ) {
							...
							$errors->add( 'validation', sprintf( __( '%s is not a valid email address.', 'ctemawp' ), '<strong>' . esc_html( $field_label ) . '</strong>' ) );
							continue;
						}
					}
					...
					if ( '' !== $data[ $key ] && in_array( 'state', $format ) ) {
						...
						if ( ! empty( $valid_states ) && is_array( $valid_states ) && sizeof( $valid_states ) > 0 ) {
							...
							if ( ! in_array( $data[ $key ], $valid_state_values ) ) {
								...
								$errors->add( 'validation', sprintf( __( '%1$s is not valid. Please enter one of the following: %2$s', 'ctemawp' ), '<strong>' . esc_html( $field_label ) . '</strong>', implode( ', ', $valid_states ) ) );
							}
						}
					}
					...
					if ( $required && '' === $data[ $key ] ) {
						/* translators: %s: field name */
						$errors->add( 'required-field', apply_filters( 'woocommerce_checkout_required_field_notice', sprintf( __( '%s is a required field.', 'ctemawp' ), '<strong>' . esc_html( $field_label ) . '</strong>' ), $field_label ) );
					}
				}
			}
		...
		public function multistep_checkout( $template, $template_name, $template_path ) {
			...
			if ( 'checkout/form-checkout.php' == $template_name ) {
				$template   = CTEMAWP_THEME_DIR . '/woocommerce/checkout/form-multistep-checkout.php';
		...
		public static function typography_settings( $settings ) {
			$settings['woo_product_title'] = array(
				'label'    => esc_html__( 'WooCommerce Product Title', 'ctemawp' ),
			...
			$settings['woo_product_price'] = array(
				'label'    => esc_html__( 'WooCommerce Product Price', 'ctemawp' ),
			...
			$settings['woo_single_add_to_cart'] = array(
				'label'    => esc_html__( 'WooCommerce Product Add To Cart', 'ctemawp' ),
			...
			$settings['woo_single_notice'] = array(
				'label'    => esc_html__( 'WooCommerce Product Notice', 'ctemawp' ),
			...
			$settings['woo_archive_title'] = array(
				'label'    => esc_html__( 'WooCommerce Archive Title', 'ctemawp' ),
			...
			$settings['woo_archive_cats'] = array(
				'label'    => esc_html__( 'WooCommerce Archive Category', 'ctemawp' ),
			...
			$settings['woo_archive_price'] = array(
				'label'    => esc_html__( 'WooCommerce Archive Price', 'ctemawp' ),
			...
			$settings['woo_product_add_to_cart'] = array(
				'label'    => esc_html__( 'WooCommerce Archive Add To Cart', 'ctemawp' ),
			...
			$settings['woo_archive_notice'] = array(
				'label'    => esc_html__( 'WooCommerce Archive Notice', 'ctemawp' ),
			...
			$settings['woo_store_notice'] = array(
				'label'    => esc_html__( 'WooCommerce Store Notice', 'ctemawp' ),
			...
			$settings['woo_store_notice_dismiss_link'] = array(
				'label'    => esc_html__( 'WooCommerce Store Notice Dismiss', 'ctemawp' ),
...
CtemaWP_WooCommerce_Config::instance();
