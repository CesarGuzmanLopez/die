 ...
 * @since 1.0.0
 */
if ( ! function_exists( 'ctemawp_wcmenucart_menu_item' ) ) {

	function ctemawp_wcmenucart_menu_item() {

		// Classes
		$classes = array( 'wcmenucart' );

		// Hide items if "hide if empty cart" is checked.
		if ( true == get_theme_mod( 'ctema_woo_menu_icon_hide_if_empty', false )
		...
		// Vars.
		$icon_style  = get_theme_mod( 'ctema_woo_menu_icon_style', 'drop_down' );
		$custom_link = get_theme_mod( 'ctema_woo_menu_icon_custom_link' );
		...
		// Cart total.
		$display = get_theme_mod( 'ctema_woo_menu_icon_display', 'icon_count' );
		...
		// Get cart icon.
		$icon = get_theme_mod( 'ctema_woo_menu_icon', 'icon_handbag' );
		...
		// If bag style.
		$woo_bag_style = get_theme_mod( 'ctema_woo_menu_bag_style', 'no' );

		if ( 'yes' === $woo_bag_style ) { ?>

			<a href="<?php echo esc_url( $url ); ?>" class="<?php echo esc_attr( $classes ); ?>">
				<?php
				if ( true == get_theme_mod( 'ctema_woo_menu_bag_style_total', false ) ) {
					?>
 ...
 * @since 1.0.0
 */
if ( ! function_exists( 'ctemawp_woo_placeholder_img' ) ) {

	function ctemawp_woo_placeholder_img() {
		if ( function_exists( 'wc_placeholder_img_src' ) && wc_placeholder_img_src() ) {
			$placeholder = '<div class="woo-entry-image clr"><img src="' . wc_placeholder_img_src() . '" alt="' . __( 'Placeholder Image', 'ctemawp' ) . '" class="woo-entry-image-main" /></div>';
			$placeholder = apply_filters( 'ctema_woo_placeholder_img_html', $placeholder );
			if ( $placeholder ) {
 ...
 * @since 1.0.0
 */
if ( ! function_exists( 'ctemawp_woo_product_instock' ) ) {

	function ctemawp_woo_product_instock( $post_id = '' ) {
		global $post;
 ...
 * @since 1.1.9
 */
if ( ! function_exists( 'ctemawp_woo_product_elements_positioning' ) ) {

	function ctemawp_woo_product_elements_positioning() {

		// Default sections
		$sections = array( 'image', 'category', 'title', 'price-rating', 'woo-rating', 'description', 'button' );

		// Get sections from Customizer
		$sections = get_theme_mod( 'ctemawp_woo_product_elements_positioning', $sections );
		...
		$sections = apply_filters( 'ctema_woo_product_elements_positioning', $sections );
 ...
 * @since 1.1.9
 */
if ( ! function_exists( 'ctemawp_woo_summary_elements_positioning' ) ) {

	function ctemawp_woo_summary_elements_positioning() {

		// Default sections
		$sections = array( 'title', 'rating', 'price', 'excerpt', 'quantity-button', 'meta' );

		// Get sections from Customizer
		$sections = get_theme_mod( 'ctemawp_woo_summary_elements_positioning', $sections );
		...
		// Apply filters for easy modification
		$sections = apply_filters( 'ctema_woo_summary_elements_positioning', $sections );
 ...
 * @since 1.8.2
 */
function ctema_is_wc_attribute() {
 ...
 * @since 2.0
 */
if ( ! function_exists( 'ctema_woo_img_link_open' ) ) {
	function ctema_woo_img_link_open() {
...
if ( ! function_exists( 'ctema_woo_img_link_close' ) ) {
	function ctema_woo_img_link_close() {
		echo '</a>';
	}
}
 ...
 * @since 2.0
 */
if ( ! function_exists( 'ctema_woo_grid_view_excerpt' ) ) {
	function ctema_woo_grid_view_excerpt() {

		global $product;

		$exc_length = get_theme_mod( 'ctema_woo_list_excerpt_length', '60' );
...
if ( ! function_exists( 'ctema_woo_wishlist_plugin' ) ) {
	 ...
	 * @since 2.0
	 */
	function ctema_woo_wishlist_plugin() {
		if ( ! CTEMAWP_WOOCOMMERCE_ACTIVE ) {
			return false;
		}

		$woo_wl_plugin = get_theme_mod( 'ctema_woo_wl_plugin', 'ti_wl' );
...
if ( ! function_exists( 'ctema_woo_wishlist' ) ) {
...
if ( ! function_exists( 'ctema_woo_wishlist_count' ) ) {
	 ...
	 * @since 2.0
	 */
	function ctema_woo_wishlist_count() {
		$wl_count = 0;

		if ( 'yith_wl' === get_theme_mod( 'ctema_woo_wl_plugin', 'ti_wl' ) && function_exists( 'yith_wcwl_count_all_products' ) ) {
...
if ( ! function_exists( 'ctema_dequeue_yith_wl_scripts' ) ) {

	/**
	 * Styles/Scripts
	 */
	function ctema_dequeue_yith_wl_scripts() {
