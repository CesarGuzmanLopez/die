...
// Get ID.
$get_id = get_theme_mod( 'ctema_error_page_template' );

// Check if page is Elementor page.
$elementor = get_post_meta( $get_id, '_elementor_edit_mode', true );

// Get content.
$get_content = ctemawp_error_page_template_content();

// If blank page.
if ( 'on' === get_theme_mod( 'ctema_error_page_blank', 'off' ) ) { ?>
										...
										if ( ! function_exists( 'elementor_theme_do_location' ) || ! elementor_theme_do_location( 'single' ) ) {
											...
											if ( ! empty( $get_id ) ) {
												...
												if ( CTEMAWP_ELEMENTOR_ACTIVE && $elementor ) {
													...
													CtemaWP_Elementor::get_error_page_content();
													...
													if ( ctema_is_block_template( $get_id ) ) {
														$get_content = apply_filters( 'ctema_error_page_template_content', do_blocks( $get_content ) );
													}
												...
												<div class="error404-content clr">
													<?php
													$logo_404 = get_theme_mod( 'ctema_404_logo' );
													if ( ! empty( $logo_404 ) ) {
														?>

														<img src="<?php echo esc_url( $logo_404 ); ?>" alt="<?php esc_attr_e( '404 Logo', 'ctemawp' ); ?>" title="<?php esc_attr_e( '404 Logo', 'ctemawp' ); ?>" />
													<?php } ?>

													<h2 class="error-title"><?php esc_html_e( 'This page could not be found!', 'ctemawp' ); ?></h2>
													<p class="error-text"><?php esc_html_e( 'We are sorry. But the page you are looking for is not available.', 'ctemawp' ); ?><br /><?php esc_html_e( 'Perhaps you can try a new search.', 'ctemawp' ); ?></p>
													<?php get_search_form(); ?>
													<a class="error-btn button" href="<?php echo esc_url( home_url( '/' ) ); ?>"><?php esc_html_e( 'Back To Homepage', 'ctemawp' ); ?></a>
...
if ( 'on' === get_theme_mod( 'ctema_error_page_blank', 'off' ) ) {
	?>
					...
					<?php do_action( 'ctema_after_main' ); ?>

				</div><!-- #wrap -->

				<?php do_action( 'ctema_after_wrap' ); ?>

			</div><!-- .outer-wrap -->

			<?php do_action( 'ctema_after_outer_wrap' ); ?>
