...
// Get ID.
$get_id = ctemawp_custom_footer_template();

// Check if page is Elementor page.
$elementor = get_post_meta( $get_id, '_elementor_edit_mode', true );

// Get content.
$get_content = ctemawp_footer_template_content();

// Get footer widgets columns.
$columns    = apply_filters( 'ctema_footer_widgets_columns', get_theme_mod( 'ctema_footer_widgets_columns', '4' ) );
$grid_class = ctemawp_grid_class( $columns );

// Responsive columns.
$tablet_columns = get_theme_mod( 'ctema_footer_widgets_tablet_columns' );
$mobile_columns = get_theme_mod( 'ctema_footer_widgets_mobile_columns' );

// Visibility.
$visibility = get_theme_mod( 'ctema_footer_widgets_visibility', 'all-devices' );

// Classes.
$wrap_classes = array( 'ctemawp-row', 'clr' );
...
// Add container class.
if ( true === get_theme_mod( 'ctema_add_footer_container', true ) ) {
...
<?php do_action( 'ctema_before_footer_widgets' ); ?>

<div id="footer-widgets" class="<?php echo esc_attr( $wrap_classes ); ?>">

	<?php do_action( 'ctema_before_footer_widgets_inner' ); ?>
	...
	<div class="<?php echo esc_attr( $inner_classes ); ?>">
		...
		if ( ! empty( $get_id ) ) {
			...
			if ( CTEMAWP_ELEMENTOR_ACTIVE && $elementor ) {
				...
				// If Elementor.
				CtemaWP_Elementor::get_footer_content();
				...
				if ( ctema_is_block_template( $get_id ) ) {
					$get_content = apply_filters( 'ctema_footer_template_content', do_blocks( $get_content ) );
				}
	...
	<?php do_action( 'ctema_after_footer_widgets_inner' ); ?>

</div><!-- #footer-widgets -->

<?php do_action( 'ctema_after_footer_widgets' ); ?>
