...
$classes      = '';
$topbar_style = get_theme_mod( 'ctema_top_bar_style', 'one' );
if ( 'one' === $topbar_style ) {
...
// Get ID.
$get_id = get_theme_mod( 'ctema_top_bar_social_alt_template' );

// Check if page is Elementor page.
$elementor = get_post_meta( $get_id, '_elementor_edit_mode', true );

// Get content.
$get_content = ctemawp_top_bar_social_alt_content();
...
if ( $get_id ) : ?>
	...
	<div id="top-bar-social-alt" class="clr <?php echo esc_attr( $classes ); ?>">
		...
		if ( CTEMAWP_ELEMENTOR_ACTIVE && $elementor ) {
			...
			CtemaWP_Elementor::get_topbar_social_alt_content();
			...
			if ( ctema_is_block_template( $get_id ) ) {
				$get_content = apply_filters( 'ctemawp_top_bar_social_alt_content', do_blocks( $get_content ) );
			}
...
$profiles = get_theme_mod( 'ctema_top_bar_social_profiles' );
if ( ! $profiles ) {
	return;
}

// Get theme mods.
$link_target = get_theme_mod( 'ctema_top_bar_social_target', 'blank' );
...
<div id="top-bar-social" class="clr <?php echo esc_attr( $classes ); ?>">
	...
	<ul class="clr" aria-label="<?php echo esc_attr__( 'Social links', 'ctemawp' ); ?>">
		...
		foreach ( $social_options as $key => $val ) {
			...
			if ( $url ) {
				...
				// Display link.
				echo '<li class="ctemawp-' . esc_attr( $key ) . '">';
