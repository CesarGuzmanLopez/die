...
// Get template.
$template = get_theme_mod( 'ctema_vertical_header_template' );
...
// Get bottom template.
$bottom_template = get_theme_mod( 'ctema_vertical_header_bottom_template' );
...
<div id="site-header-inner" class="<?php echo esc_attr( $classes ); ?>">
	...
	if ( ! empty( $template ) ) {
		...
		if ( CTEMAWP_ELEMENTOR_ACTIVE && $elementor ) {
			...
			// If Elementor.
			CtemaWP_Elementor::get_vertical_header_content();
			...
			if ( ctema_is_block_template( $template ) ) {
				$get_content = apply_filters( 'ctemawp_vertical_header_content', do_blocks( $get_content ) );
			}
		...
		<?php
		// Search form.
		if ( true === get_theme_mod( 'ctema_vertical_header_search_form', true ) ) {
		...
		if ( ! empty( $bottom_template ) ) {
			...
			if ( CTEMAWP_ELEMENTOR_ACTIVE && $bottom_elementor ) {

				// If Elementor.
				CtemaWP_Elementor::get_vertical_header_bottom_content();

			} else if ( CTEMAWP_BEAVER_BUILDER_ACTIVE && ! empty( $bottom_template ) ) {
				...
				// If Gutenberg.
				if ( ctema_is_block_template( $bottom_template ) ) {
					$get_bottom_content = apply_filters( 'ctemawp_vertical_header_bottom_content', do_blocks( $get_bottom_content ) );
				}
